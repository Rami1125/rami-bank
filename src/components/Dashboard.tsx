...
export default function Dashboard({ 
  ...
  const [editCategory, setEditCategory] = useState('מזון וסופרמרקט');
  ...
  const handleOpenEditModal = (tx: Transaction) => {
    ...
    setEditVendorName(tx.vendorName);
    setEditFreeText(tx.freeText || '');
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    if (isNaN(parsedAmount) || parsedAmount <= 0) {
      alert("נא להזין סכום תקין הגדול מאפס");
      return;
    }
    ...
    try {
      const updated: Transaction = {
        ...
        vendorName: editVendorName.trim() || 'אחר',
        freeText: editFreeText.trim(),
        date: editDate,
        type: editType
      };
            ...
            transactions.slice(0, 10).map((tx) => (
              <div key={tx.id} className="p-3.5 flex justify-between items-center group hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 focus-within:opacity-100 transition-opacity duration-300">
                    <button 
                      type="button"
                      onClick={() => handleOpenEditModal(tx)}
                      className="p-1.5 border border-slate-100 rounded-lg text-slate-400 hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-600 transition-all cursor-pointer"
                      title="ערוך עסקה"
                    >
                    ...
                    <button 
                      type="button"
                      onClick={() => handleDeleteTrigger(tx)}
                      className="p-1.5 border border-slate-100 rounded-lg text-slate-400 hover:border-rose-300 hover:bg-rose-50 hover:text-rose-600 transition-all cursor-pointer"
                      title="מחק עסקה"
                    >
        ...
        {editingTransaction && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" dir="rtl">
            ...
            >
              ...
              <form onSubmit={handleSaveEdit} className="space-y-4 font-sans text-xs font-semibold text-slate-700">
                <div className="grid grid-cols-2 gap-3.5">
                  <div>
                    <label className="block mb-1 text-slate-600 font-medium">סוג עסקה</label>
                    <select
                      value={editType}
                  ...
                  <div>
                    <label className="block mb-1 text-slate-600 font-medium">סכום (₪)</label>
                    <input
                      type="number"
                      required
                      step="0.01"
                      placeholder="₪ סכום העסקה"
                      value={editAmount}
                ...
                <div>
                  <label className="block mb-1 text-slate-600 font-medium">קטגוריה</label>
                  <select
                    value={editCategory}
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  <div>
                    <label className="block mb-1 text-slate-600 font-medium">שם בית עסק / מקור</label>
                    <input
                      type="text"
                      required
                      placeholder="לדוג׳: שופרסל, דלק"
                  ...
                  <div>
                    <label className="block mb-1 text-slate-600 font-medium">תאריך ביצוע</label>
                    <input
                      type="date"
                      required
                      value={editDate}
                      onChange={(e) => setEditDate(e.target.value)}
                      className="w-full p-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white text-xs font-mono text-center"
                    />
                  </div>
                </div>

                <div>
                  <label className="block mb-1 text-slate-600 font-medium">תיאור קצר</label>
                  <input
                    type="text"
                    placeholder="הערה קצרה על העסקה"
                    value={editFreeText}
                ...
                <button
                  type="submit"
                  className="w-full text-white bg-emerald-600 hover:bg-emerald-700 py-3 rounded-xl text-sm font-bold shadow-md shadow-emerald-600/10 transition-colors cursor-pointer mt-2"
                >
                  שמור שינויים בעסקה
                </button>
              </form>
          ...
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" dir="rtl">
            ...
            >
              <div className="text-center space-y-2">
                ...
                <h3 className="text-sm font-extrabold text-slate-900">אישור מחיקת עסקה</h3>
                <p className="text-xs text-slate-500 font-medium leading-relaxed">
                  האם אתה בטוח שברצונך למחוק את העסקה עבור <strong className="text-rose-650">"{transactionToDelete.vendorName}"</strong> בסך <strong className="text-rose-650">{transactionToDelete.amount.toLocaleString()} ₪</strong>? פעולה זו תגרע את העסקה לצמיתות ותעדכן את מסגרת התקציב.
                </p>
              </div>
